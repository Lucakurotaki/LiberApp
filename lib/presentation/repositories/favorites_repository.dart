...
class FavoritesRepository extends BaseRepository{
  ...
  _readFavorites() async{
    if(auth.appUser != null && _list.isEmpty){
      final snapshot = await db.collection('users/${auth.appUser!.uid}/favorites').get();
  ...
  save(Book book)async{
    if(!_list.any((element) => element.id == book.id)){
      _list.add(book);
      await db.collection('users/${auth.appUser!.uid}/favorites').doc(book.id).set(
        {'id': book.id,
        'title' : book.title,
        'author' : book.author,
  ...
  remove(Book book)async {
    await db.collection('users/${auth.appUser!.uid}/favorites').doc(book.id).delete();
    _list.remove(book);
    notifyListeners();
  }
